...
def decode_sentences(sentences, vocab):
    output = []
    for s in sentences:
        ids = [vocab["<SOS>"]] + [vocab.get(w, vocab["<UNK>"]) for w in tokenize_morphs(s)]
        ids = ids[:MAX_SEQ] + [vocab["<PAD>"]] * (MAX_SEQ - len(ids))
        output.append(ids)
    return output
...
def label_sentences(sentences, vocab):
    output = []
    for s in sentences:
        ids = [vocab.get(w, vocab["<UNK>"]) for w in tokenize_morphs(s)]
        ids = ids[:MAX_SEQ - 1] + [vocab["<END>"]]
        ids += [vocab["<PAD>"]] * (MAX_SEQ - len(ids))
        output.append(ids)
    return output